...
class EnvironmentAgent:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # 音乐播放器（支持环境变量）
        music_dir = self.config.get('music_dir', os.getenv('MUSIC_DIR', '/Users/m3airmima0000/Mind-Daemon/music'))
        self.music_player = MusicPlayer(music_dir)
        
        # 光晕控制器（支持环境变量）
        window_py_path = self.config.get('window_py_path', os.getenv('WINDOW_PY_PATH', '/Users/m3airmima0000/Desktop/window.py'))
        self.halo_controller = HaloController(window_py_path)
        
        # LLM API配置（支持环境变量）
        self.minimax_api_key = self.config.get('minimax_api_key', os.getenv('MINIMAX_API_KEY', ''))
        self.minimax_base_url = self.config.get('minimax_base_url', os.getenv('MINIMAX_BASE_URL', 'https://api.minimax.chat/v1/text/chatcompletion_v2'))
    ...
    def _generate_mock_decision(self, prompt: str) -> str:
        ...
        if "STRESSED" in prompt or "PRESSURE" in prompt:
            return json.dumps({
                ...
                "halo": {"action": "activate", "color": [255, 200, 150], "reason": "压力缓解"}
            })
            ...
            return json.dumps({
                ...
                "curtain": {"action": "open"},
                "halo": {"action": "minimal", "color": [100, 150, 255], "reason": "专注辅助"}
            })
            ...
            return json.dumps({
                ...
                "music": {"action": "switch", "type": "relax"},
                "curtain": {"action": "close"},
                "halo": {"action": "activate", "color": [255, 182, 193], "reason": "疲劳恢复"}
            })
...
4. 光晕控制（智能颜色选择）：
   - action: "activate"(激活), "soft"(柔和显示), "attention"(注意力提醒), "minimal"(最小化), "maintain"(保持)
   - color: [R, G, B] RGB颜色数组，请根据心理学原理选择合适的颜色
   - reason: 简短说明使用此颜色的原因
...
控制原则：
- 压力/焦虑状态：温暖色调灯光，切换放松音乐，关闭窗帘，激活舒缓色光晕
- 专注状态：明亮白光，切换专注音乐，打开窗帘，最小化蓝色光晕或不使用
- 疲劳状态：柔和粉色灯光，保持或切换到放松音乐，关闭窗帘，激活温暖色光晕
- 放松状态：柔和紫色灯光，保持放松音乐，适当开窗，柔和光晕或不使用
- 分心状态：清爽灯光，切换专注音乐，打开窗帘，黄色注意力提醒光晕
...
```"""
    ...
    def get_current_environment_state(self) -> Dict[str, Any]:
        """获取当前环境状态"""
        # 更新音乐播放状态
        if self.music_state.is_playing and not self.music_player.is_track_playing():
