...
class GlowWindow(QWidget):
    def __init__(self, idle_timeout=5, color_rgb=(153, 153, 255), disable_auto=False):  # 默认5s空闲时间，默认紫蓝色
        super().__init__()
        
        # --- 空闲检测参数 ---
        self.idle_timeout = idle_timeout  # 空闲超时时间（秒）
        self.is_auto_mode = not disable_auto  # 是否启用自动模式（可以通过参数禁用）
        ...
        # --- 光晕效果参数 ---
        # 使用传入的颜色参数，支持从环境控制系统动态设置
        self.glow_color = QColor(color_rgb[0], color_rgb[1], color_rgb[2])
        print(f"光晕初始化 - 颜色: RGB{color_rgb}") 
        # 边框的厚度，可以根据喜好调整
    ...
    def check_idle_status(self):
        ...
        elif idle_time < 2 and self.is_glow_active:  # 2秒内有活动视为恢复（更敏感）
    ...
    def activate_glow(self):
        ...
        print(f"光晕激活 - 颜色: RGB({self.glow_color.red()}, {self.glow_color.green()}, {self.glow_color.blue()})")
    ...
    def manual_toggle(self):
        ...
        if self.is_glow_active:
            ...
            self.activate_glow()
        print(f"手动切换光晕: {'激活' if self.is_glow_active else '停用'}")

    def set_glow_color(self, r, g, b):
        """外部调用此函数来更新光晕颜色"""
        self.glow_color.setRgb(r, g, b)
        print(f"光晕颜色已更新为: RGB({r}, {g}, {b})")
...
if __name__ == '__main__':
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='屏幕边缘光晕效果')
    parser.add_argument('--color', type=str, default='153,153,255', 
                        help='光晕颜色，格式: R,G,B 或 #RRGGBB （默认: 153,153,255）')
    parser.add_argument('--timeout', type=int, default=1,
                        help='空闲超时时间（秒）（默认: 1）')
    parser.add_argument('--disable-auto', action='store_true',
                        help='禁用自动空闲检测，仅响应外部控制')
    args = parser.parse_args()
    
    app = QApplication(sys.argv)
    
    # 解析颜色参数
    color_rgb = parse_color_arg(args.color)
    
    # 创建光晕窗口，使用命令行参数
    window = GlowWindow(idle_timeout=args.timeout, color_rgb=color_rgb, disable_auto=args.disable_auto)
    
    print(f"光晕颜色设置为: RGB{color_rgb}")
    print(f"自动模式: {'禁用' if args.disable_auto else '启用'}")
